      ...
      <div v-if="restockingOrders.length > 0" class="card restocking-section">
        ...
        <div class="table-container">
          <table class="orders-table">
            <thead>
              <tr>
                <th class="col-order-number">{{ t('restocking.columns.orderNumber') }}</th>
                <th class="col-items">{{ t('restocking.columns.items') }}</th>
                <th class="col-date">{{ t('restocking.columns.dateSubmitted') }}</th>
                <th class="col-lead">{{ t('restocking.columns.leadTime') }}</th>
                <th class="col-date">{{ t('restocking.columns.expectedDelivery') }}</th>
                <th class="col-value">{{ t('restocking.columns.totalValue') }}</th>
            ...
            <tbody>
              <tr v-for="ro in restockingOrders" :key="ro.id">
                ...
                <td class="col-lead">{{ ro.lead_time_days }} days</td>
                <td class="col-date">{{ formatDate(ro.expected_delivery) }}</td>
                <td class="col-value"><strong>{{ currencySymbol }}{{ ro.total_value.toLocaleString() }}</strong></td>
                <td class="col-status">
                  <span class="badge restocking">{{ ro.status }}</span>
                </td>
              </tr>
            </tbody>
          </table>
